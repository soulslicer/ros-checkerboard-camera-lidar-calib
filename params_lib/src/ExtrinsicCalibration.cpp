...
ExtrinsicCalibration::ExtrinsicCalibration(const std::vector<int>& boardSize, float squareSize){
    boardSize_ = cv::Size(boardSize[0], boardSize[1]);
    squareSize_ = squareSize;
    pattern_ = Pattern::CHESSBOARD;
...
std::vector<Eigen::MatrixXd> ExtrinsicCalibration::calibrateExtrinsics (const cv::Mat& img, cv::Mat& drawImg, cv::Mat& extMat, bool draw)
{
    int squareShift = 0;
    float heightShift = 0;
    Eigen::Vector3f shift(0., 0., 0.);
    Eigen::MatrixXd transPointsEig;
    Eigen::MatrixXd imagePointsEig;
    ...
    if (pattern_ == CHESSBOARD && found) {
        ...
        if(draw) cv::drawChessboardCorners(drawImg, boardSize_, cv::Mat(pointbuf), found);

        // Copy to Eigen
        imagePointsEig.resize(imagePoints.size(), 2);
        for(int j=0; j<imagePoints.size(); j++){
            imagePointsEig(j,0) = imagePoints[j].x;
            imagePointsEig(j,1) = imagePoints[j].y;
        }
        ...
        if (imagePoints.size() > 0) {
            ...
            if(extMat.rows !=4 && extMat.cols !=4)
                throw std::runtime_error("extMat is not a 4x4 matrix");
            ...
            // Check calibration
            if(draw) checkCalibration(drawImg, extMat);

            // 3D Points
            Eigen::MatrixXd extMatEig(4,4);
            cv::cv2eigen(extMat, extMatEig);
            Eigen::MatrixXd objectPointsEig(4, objectPoints.size());
            for(int i=0;  i<objectPoints.size(); i++){
                const cv::Point3f& oP = objectPoints[i];
                objectPointsEig(0, i) = oP.x;
                objectPointsEig(1, i) = oP.y;
                objectPointsEig(2, i) = oP.z;
                objectPointsEig(3, i) = 1;
            }
            transPointsEig = (extMatEig * objectPointsEig).transpose();
        }
    ...
    return {transPointsEig, imagePointsEig};
}
